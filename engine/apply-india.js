...
async function fetchViaApi(url) {
  ...
  if (m) {
    // companies.js used to sit beside this file; it now lives per-letter under
    // data/. Search every letter rather than assuming one.
    const P = require('./paths');
    let cfg = null;
    for (const L of P.LETTERS) {
      try {
        cfg = require(require('path').join(P.letterDir(L), 'companies.js'))
          .find((c) => c.ats && c.ats.type === 'cvviz' && c.ats.slug === m[1]);
      } catch { /* letter not present */ }
      if (cfg) break;
    }
